...
//Shader class
	...
	{
		std::string vertexShaderSource = eh::loadShaderSourceFromFile(vertexShader.c_str());
		std::string fragmentShaderSource = eh::loadShaderSourceFromFile(fragmentShader.c_str());
	...
	{
		glUniform2f(glGetUniformLocation(m_id, name.c_str()),
			x,
			y);
	...
	void Shader::setMat4(const std::string& name, const eh::Mat4& v) const
	{
